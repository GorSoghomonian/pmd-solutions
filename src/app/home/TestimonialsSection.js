...
export default function TestimonialsSection({ className = '' } = {}) {
  ...
  return (
    <div className={`relative bg-white rounded-t-3xl shadow-2xl -mt-10 z-10 overflow-x-hidden ${className}`}>
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="max-w-4xl mx-auto mb-16">
            <div className="overflow-hidden" ref={emblaRef}>
              <div className="flex">
                {slides.map((s, i) => (
                  <div key={i} className="min-w-0 flex-[0_0_100%] px-0 group rounded-3xl overflow-hidden bg-white shadow-[0_8px_30px_rgba(0,0,0,0.06)] hover:shadow-[0_12px_40px_rgba(42,115,221,0.15)]  transform transition-transform duration-300 hover:-translate-y-1">
                    <div
                      ref={(el) => (cardRefs.current[i] = el)}
                      className="bg-gradient-to-br from-[#2A73DD] to-blue-800 rounded-3xl p-12 text-center text-white shadow-2xl min-h-[280px]"
              ...
              {slides.map((s, i) => (
                <button
                  ...
                  className={`px-4 py-2 rounded-full border transition-all duration-300 ${
                    index === i
                      ? 'bg-white text-[#2A73DD] border-white shadow-md'
                      : 'bg-white/10 text-white border-white/30 hover:bg-white/20'
                  }`}
                  title={`${s.author || baseAuthor} — ${s.company || baseCompany}`}
                >
                  {s.company || `Case ${i + 1}`}
                </button>
              ))}
          ...
          <div className="max-w-5xl mx-auto pt-8 border-t border-gray-100">
